...
app.get("/api", (req, res) => {
  Promise.all([getBusLines(), getStopNames()])
    ...
    .catch((error) => {
      console.log(error);
      res.status(500).send("Could not get bus lines and stops");
    });
});
...
function getBusLines() {
  return axios({
    method: "get",
    url: `https://api.sl.se/api2/LineData.json?model=jour&key=${API_KEY}&DefaultTransportModeCode=BUS`,
...
function getStopNames() {
  return axios({
    ...
    const stopNames = Object.fromEntries(
      result.map((stop) => [stop.StopPointNumber, stop.StopPointName])
    );
